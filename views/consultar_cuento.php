...
if($usuario_id) {
    ...
    if ($result->num_rows > 0) {
        $puede_continuar = false;
        $mensaje_colaboracion = "Ya has colaborado en este cuento.";
//        echo '<p>Ya has colaborado en este cuento pero puedes empezar otro</p>';
    ...
    while ($row = $result->fetch_assoc()) {
        $cuentos_colaborados[] = $row['cuento_id'];
    }
    $stmt->close();
...
// Obtener el último fragmento añadido al cuento
$query = "SELECT fragmento
          FROM colaboraciones
          WHERE cuento_id = ?
          ORDER BY fecha DESC
          LIMIT 1";
...
if ($result->num_rows > 0) {
    ...
    $ultimo_fragmento = $row['fragmento'];
}
$stmt->close();

// Cortar las últimas 50 palabras del último fragmento
$palabras = explode(' ', $ultimo_fragmento);
$ultimas_50_palabras = array_slice($palabras, -50);
$texto_a_mostrar = implode(' ', $ultimas_50_palabras);
...
<header>
    ...
    <?php if($mensaje_colaboracion): ?>
        <p class="info-message"><?php echo htmlspecialchars($mensaje_colaboracion)?></p>
    <?php endif ?>
    ...
    <div id="listado-cuentos">
        ...
        <ul>
            <?php foreach ($cuentos as $cuento): ?>
                <li onclick="cargarCuento(<?php echo $cuento['id']; ?>)">
                    ...
                    <?php if (in_array($cuento['id'], $cuentos_colaborados)): ?>
                        <span class="info-message">Ya has colaborado</span>
                    <?php endif; ?>
                </li>
            <?php endforeach; ?>
        </ul>
        ...
        <div id="marco-cuento">
            ...
            <p id="texto-cuento">
                <p><?php echo nl2br(htmlspecialchars($texto_a_mostrar?? 'No disponible')); ?></p>
            </p>
        </div>
    <main>
...
<        
        ...
        <!-- Contenedor para el mensaje dinámico -->
         <p id="mensaje-colaboracion" class="info-message" style="display:none;"></p>
        ...
        <div id="formulario-continuar" style="display: none;">
            <form action="../php/continuar_cuento2.php" method="POST">
    ...
    <script>
        ...
        function verificarUsuario() {        
            const mensajeColaboracion = document.getElementById('mensaje-colaboracion');
            ...
            if (usuarioLogueado) {
                if (puedeContinuar) {
                    ...
                    mensajeColaboracion.textContent = "Ya has colaborado en este cuento.";
                    mensajeColaboracion.style.display = "block";
